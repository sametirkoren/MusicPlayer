...
function Player() {
    ...
    return (
        <div className="bg white absolute bottom-0 left-0 right-0 p-5 shadow-lg bg-gray-100 border">
            <div className="flex justify-between items-center">
                <div className="flex items-center gap-2 w-96">
                    <img className="h-20 w-32" alt=""
                         src="https://freepngimg.com/thumb/music/24394-5-music-photos-thumb.png"/>
                    <div>
                ...
                <div className="w-96 flex flex-col items-center">
                    ...
                    <div className="flex gap-3">
                        <i className="ri-skip-back-line text-4xl" onClick={onPrev}></i>
                        {isPlaying ? (<i className="ri-pause-line text-4xl" onClick={onPause}></i>) : (
                            <i className="ri-play-line text-4xl" onClick={onPlay}></i>)}
                        <i className="ri-skip-forward-line text-4xl" onClick={onNext}></i>
                    </div>
                    <div className="flex gap-3 items-center w-full">
                        <i className={`ri-shuffle-line ${shuffleOn && 'text-orange-500 font-semibold'}`} onClick={() => {
                ...
                <div className="flex gap-3 items-center">
                    <i className="ri-volume-mute-line text-4xl" onClick={() => {
                        setVolume(0);
                        audioRef.current.volume = 0;
                    }}></i>
                    <input type="range" min={0} max={1} step={0.1} className="p-0" value={volume} onChange={(e) => {
                        audioRef.current.value = e.target.value;
                        setVolume(e.target.value);
                    }}/>
                    <i className="ri-volume-down-line text-4xl" onClick={() => {
                        setVolume(1);
